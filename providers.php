...
if(!isset($CRON_GUVENLIK)){
    echo "cron dosyasını manuel çalıştıramazsınız";
    exit;
}
...
$there_change=0;
        ...
        if( $service["api_service"] == $apiService["service"] ):
          ...
          $extras = json_decode($service["api_detail"],true);
            if( $apiService["rate"] != $extras["rate"] ):
              ...
              $insert->execute(array("service"=>$service["service_id"],"alert"=>"#".$service["service_id"]." numaralı ".$service["service_name"]." isimli servisin fiyatı değiştirilmiş.","date"=>date("Y-m-d H:i:s"),"extra"=>json_encode($extra) ));
              if( $insert ): $there_change = $there_change+1; endif;
            endif;
            if( $apiService["min"] != $extras["min"] ):
              $extra  = ["old"=>$extras["min"],"new"=>$apiService["min"] ];
              $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
              $insert->execute(array("service"=>$service["service_id"],"alert"=>"#".$service["service_id"]." numaralı ".$service["service_name"]." isimli servisin minimum miktarı değiştirilmiş.","date"=>date("Y-m-d H:i:s"),"extra"=>json_encode($extra) ));
              if( $insert ): $there_change = $there_change+1; endif;
            endif;
            if( $apiService["max"] != $extras["max"] ):
              $extra  = ["old"=>$extras["max"],"new"=>$apiService["max"] ];
              $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
              $insert->execute(array("service"=>$service["service_id"],"alert"=>"#".$service["service_id"]." numaralı ".$service["service_name"]." isimli servisin maksimum miktarı değiştirilmiş.","date"=>date("Y-m-d H:i:s"),"extra"=>json_encode($extra) ));
              if( $insert ): $there_change = $there_change+1; endif;
            endif;
  ...
  if( $settings["alert_serviceapialert"] == 2 && $there_change ):
    ...
    if( $sendsms ):
        $rand = rand(1,99999);  
      SMSUser($settings["admin_telephone"],"Servis sağlayıcı tarafından bilgisi değişen servisleriniz mevcut, kontrol etmeniz önerilir.".$rand);
    endif;
    if( $sendmail ):
      sendMail(["subject"=>"ℹ️ Sağlayıcı Bilgisi","body"=>"Servis sağlayıcısı tarafından bilgisi değişen servisleriniz mevcut, kontrol etmeniz önerilir.","mail"=>$settings["admin_mail"]]);
    endif;
  endif;
