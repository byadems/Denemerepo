...
elseif ($action == "search_all_services"):
        ...
        if ($categoryRow["category_secret"] == 2 || $search->rowCount()) {
            ...
            foreach ($services as $service) {
                ...
                if ($service["service_secret"] == 2 || $searchService->rowCount()) {
                    ...
                    $matchSource = mb_strtolower($service["service_id"] . " " . $name, "UTF-8");
                    if ($keyword === "" || mb_strpos($matchSource, $keyword) !== false) {
                        $serviceList .= "<option value='" . $service['service_id'] . "' data-category='" . $categoryRow["category_id"] . "'>" . $service["service_id"] . " - " . $name . " - " . priceFormat(service_price($service["service_id"])) . $currency . "</option>";
                    }
                }
            }
    ...
    echo json_encode(['services' => $serviceList, 'hasService' => !empty($serviceList)]);
...
<input class="form-control" name="link" value="" type="text" id="field-orderform-fields-link" maxlength="105">
</div>';
    endif;
...
<small class="help-block min-max">Minimum:<span style="color: red;"> ' . number_format($service["service_min"], 0, '.', '.') . '</span> - Maksimum:<span style="color: green;"> ' . number_format($service["service_max"], 0, '.', '.') . '</span></small>
';
    endif;
