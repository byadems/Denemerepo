  ...
  if( $_POST ){
    ...
    $message  = htmlentities($_POST["message"]);
      ...
      if($post){
        ...
        $insert5= $insert5->execute(array("c_id"=>$user["client_id"],"action"=>"Destek talebi <strong>Otomatik</strong> olarak cevaplandı. ID:".$ticket_id,"ip"=>GetIP(),"date"=>date("Y-m-d H:i:s") ));
      }
     ...
     if( $settings["alert_newticket"] == 2 ):
            ...
            if( $sendsms ):
              SMSUser($settings["admin_telephone"],"Sitenizde #".$ticket_id." idli yeni bir destek talebi mevcut!");
            endif;
            if( $sendmail ):
              sendMail(["subject"=>"💬 Yeni destek talebi mevcut","body"=>"Sitenizde #".$ticket_id." idli yeni bir destek talebi mevcut! İlgilenecek misiniz acaba? 🙄","mail"=>$settings["admin_mail"]]);
            endif;
          endif;
        else:
          $error    = 1;
  ...
  if( $_POST ){
   ...
   $message  = htmlspecialchars($_POST["message"]);
      if( strlen(str_replace(' ','',$message)) < 5 ){
        ...
        $insert3= $conn->prepare("INSERT INTO client_report SET client_id=:c_id, action=:action, report_ip=:ip, report_date=:date ");
        $insert3= $insert3->execute(array("c_id"=>$user["client_id"],"action"=>"Destek talebi cevaplandı #".route(1),"ip"=>GetIP(),"date"=>date("Y-m-d H:i:s") ));
