<?php
  ...
  elseif( $action == "new" ):
    ...
    if($settings["guard_roles_status"] == 2 && $settings["guard_system_status"] == 2){
        ...
        if( !email_check($email) ){
          $error      = 1;
          $errorText  = "Lütfen geçerli bir e-posta formatı giriniz!";
          $icon     = "error";
        }elseif( userdata_check("email",$email) ){
          $error      = 1;
          $errorText  = "Girdiğiniz e-posta adresi kullanılıyor.";
          $icon     = "error";
        }elseif( !username_check($username) ){
          $error      = 1;
          $errorText  = "Kullanıcı adı içerisinde harf ve sayılar içeren en az 4 en fazla 32 karakter olmalıdır.";
          $icon     = "error";
        }elseif( userdata_check("username",$username) ){
          $error      = 1;
          $errorText  = "Girdiğiniz kullanıcı adı kullanılıyor!";
          $icon     = "error";
    ...
    $client_access  = json_decode($client_detail["access"],true);
        if( $_POST ):
          ...
          if( strlen($password) < 8 ){
            $error      = 1;
            $errorText  = "Şifre en az 8 karakterli olmalı!";
            $icon       = "error";
          }else{
            $conn->beginTransaction();
  ...
  elseif( $action == "del_price" ):
    $client_id  = route(3);
    if( !countRow(["table"=>"clients_price","where"=>["client_id"=>$client_id]]) ): $_SESSION["client"]["data"]["error"]    = 1; $_SESSION["client"]["data"]["errorText"]= "Üyeye ait fiyatlandırma bulunamadı."; header("Location:".site_url("admin/clients")); exit(); endif;
  ...
  setcookie("u_id", 'acılara yürüyor korkmuyorum', time()-(60*60*24*7), '/', null, null, true );
  setcookie("u_password", 'arada bir kalbini yokluyorum', time()-(60*60*24*7), '/', null, null, true );
  setcookie("u_login", 'cennetten çiçek mi topluyorum', time()-(60*60*24*7), '/', null, null, true );
  session_destroy();
